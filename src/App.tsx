...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed.length > 0) {
          setChampionScore(parsed[0].score);
        }
      } catch (e) {
        console.error(e);
      }
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-100 flex flex-col font-sans relative antialiased overflow-x-hidden selection:bg-[#D4AF37] selection:text-black" id="main-root">
      ...
      {/* Header Bar */}
      <header className="border-b border-white/5 bg-black/80 backdrop-blur-md sticky top-0 z-30 px-4 py-3" id="app-header">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="bg-gradient-to-br from-[#D4AF37] to-[#8a6f27] p-2 rounded-xl shadow-lg ring-1 ring-white/10" id="brand-logo">
              <Sparkles className="w-5 h-5 text-black animate-pulse" />
            </div>
            <div>
              <h1 className="text-xl font-display italic font-semibold tracking-wide text-white leading-none">
                L'Arcade du Match
              </h1>
              <span className="text-[10px] text-zinc-500 font-semibold font-mono block mt-0.5 uppercase tracking-widest">
          ...
          <div className="flex items-center gap-2">
            {(gameState === 'START' || gameState === 'GAMEOVER') && (
              <>
                <button
                  ...
                  className="bg-white/5 hover:bg-white/10 text-[#D4AF37] border border-[#D4AF37]/30 hover:border-[#D4AF37]/60 font-bold font-display italic text-xs px-3 py-1.5 rounded-lg flex items-center gap-1 transition-all shadow-md active:scale-95 cursor-pointer"
                ...
                <button
                  ...
                  className="bg-[#D4AF37] hover:bg-[#bfa032] text-black font-bold font-display italic text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all shadow-md active:scale-95 cursor-pointer"
            ...
            {gameState === 'PLAYING' && (
              <button
                ...
                className="bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 text-zinc-300 font-semibold text-xs px-3 py-1.5 rounded-lg flex items-center gap-1 transition-all"
                id="pause-game-btn"
              >
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 md:p-6 flex flex-col justify-center" id="app-main">
        {/* State: START / Title Menu Screen */}
        {gameState === 'START' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start" id="start-screen">
            {/* Left Hand: App Details & Mode Tuning */}
            <div className="lg:col-span-7 col-span-1 space-y-6">
              
              {/* Introduction Card */}
              <div className="bg-[#0F0F0F]/80 backdrop-blur-md rounded-2xl border border-white/10 p-6 space-y-4 shadow-xl">
                ...
                <p className="text-zinc-400 text-sm leading-relaxed font-sans mt-2">
                  縦か横に同じデザインの宝石を3つ以上並べて消していくシンプルなパズルゲームです。
                      ...
                      <button
                        ...
                        className={`py-2 rounded-xl text-xs font-bold border transition-all flex flex-col items-center justify-center gap-0.5 ${
                          roundDuration === secs
                            ...
                            : 'bg-white/5 border-white/5 text-zinc-450 text-zinc-400 hover:bg-white/10 hover:text-white'
                        }`}
                        id={`round-dur-${secs}`}
                      >
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  onClick={handleStartTestPlay}
                  className="w-full bg-white/5 hover:bg-white/10 text-[#D4AF37] border border-[#D4AF37]/30 hover:border-[#D4AF37]/60 font-bold font-display italic text-base sm:text-lg py-4 rounded-xl transition-all shadow-md select-none cursor-pointer flex items-center justify-center gap-2"
                ...
                <button
                  onClick={handleStartGame}
                  className="w-full bg-[#D4AF37] hover:bg-[#bfa032] text-black font-bold font-display italic text-base sm:text-lg py-4 rounded-xl transition-all shadow-xl shadow-[#D4AF37]/10 hover:shadow-[#D4AF37]/20 select-none cursor-pointer flex items-center justify-center gap-2"
                  id="main-start-btn"
                >
                ...
                <div className="bg-gradient-to-br from-[#D4AF37]/5 to-[#D4AF37]/10 border border-[#D4AF37]/30 rounded-2xl p-5 shadow-lg relative overflow-hidden" id="champion-badge">
                  ...
                  <div className="flex gap-4 items-center animate-combo-pulse">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-zinc-450 text-zinc-400 font-sans mt-0.5">連鎖倍率を調整して最高の称号を掴み取ろう！</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        {(gameState === 'PLAYING' || gameState === 'PAUSED') && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start" id="active-game-screen">
            
            {/* Grid Board container */}
            <div className="lg:col-span-7 col-span-1 flex flex-col items-center">
            ...
            <div className="lg:col-span-5 col-span-1 space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs text-zinc-400 px-1">
                  ...
                  <span className="text-[10px] text-zinc-500 font-mono">※ リアルタイム調整可</span>
                </div>
                <TuningPanel setting={multiplierSetting} onChange={setMultiplierSetting} />
              </div>
            ...
            <div className="bg-[#0F0F0F]/90 backdrop-blur-md rounded-2xl border border-white/10 p-8 shadow-2xl space-y-5 text-center relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center items-center pt-2">
                ...
                <button
                  ...
                  className="bg-white/5 hover:bg-white/10 text-zinc-300 font-semibold py-3 px-8 rounded-xl transition-all border border-white/10"
      ...
      {/* Footer credits bar */}
      <footer className="border-t border-white/5 bg-[#050505] px-4 py-4 text-center mt-auto text-[10px] text-zinc-600 font-mono flex flex-col sm:flex-row justify-between items-center gap-2 max-w-6xl w-full mx-auto" id="app-footer">
