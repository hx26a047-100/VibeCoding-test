...
export default function GameBoard({ board, disabled, onSwapAttempt, isShuffling }: GameBoardProps) {
  ...
  return (
    <div className="relative w-full aspect-square max-w-[480px] mx-auto bg-black/40 backdrop-blur-md rounded-2xl border border-white/10 p-2.5 shadow-[0_0_40px_rgba(0,0,0,0.6)] overflow-hidden select-none" id="game-board-container">
      ...
      <div className="absolute inset-2.5 grid grid-cols-7 grid-rows-7 gap-1.5 pointer-events-none" id="background-grid-guide">
      ...
      <div className="absolute inset-2.5 grid grid-cols-7 grid-rows-7 gap-1.5" id="interactive-tiles-layer">
        {board.map((row, r) =>
          row.map((tile) => {
            ...
            return (
              ...
              >
                <TileIcon className="w-6 h-6 md:w-8 md:h-8 drop-shadow-md animate-enter transition-transform duration-300 group-hover:scale-110" />
              </div>
            );
          })
        )}
