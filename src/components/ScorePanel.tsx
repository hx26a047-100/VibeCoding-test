...
export default function ScorePanel({
  ...
  return (
    <div className="bg-[#0F0F0F]/90 backdrop-blur-md rounded-2xl border border-white/10 p-5 shadow-2xl space-y-4" id="score-panel">
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Score Counter */}
        <div className="bg-black/40 rounded-xl p-3.5 border border-white/5 flex flex-col justify-center">
          <span className="text-xs font-display italic text-[#D4AF37] tracking-wide block">Current Score</span>
          <span
            className={`text-2xl font-mono font-bold tracking-tight mt-1 transition-all duration-150 ${
        ...
        <div className="bg-black/40 rounded-xl p-3.5 border border-white/5 flex flex-col justify-center">
          ...
          <div className="flex items-baseline gap-1 mt-1">
            <span className="text-2xl font-mono font-bold text-[#D4AF37]" id="multiplier-display">
      ...
      {/* Time Remaining Bar */}
      <div className="space-y-1.5" id="timer-bar-group">
        <div className="flex justify-between items-center text-xs">
          <span className="font-display italic text-[#D4AF37] tracking-wider flex items-center gap-1">
            <Clock className={`w-4 h-4 ${isTimeLow ? 'text-red-500 animate-bounce' : 'text-[#D4AF37]'}`} />
            <span>Time Remaining</span>
          </span>
          <span className={`font-mono font-bold ${isTimeLow ? 'text-red-500 animate-pulse text-lg' : 'text-zinc-200'}`}>
            {isTestPlay ? '無制限' : `${timeRemaining}秒`}
          </span>
        </div>
        <div className="h-3 bg-black rounded-full overflow-hidden border border-white/5 p-[1.5px]">
          <div
      ...
      <div className="bg-black/30 rounded-xl p-3 border border-white/5 grid grid-cols-3 gap-2 text-center" id="advanced-stats-grid">
        <div className="space-y-0.5">
          <span className="text-[9px] text-zinc-500 uppercase tracking-widest font-semibold font-mono">Max Combo</span>
          <div className="font-mono text-sm font-bold text-white">{stats.maxCombo}回</div>
        </div>
        <div className="space-y-0.5 border-x border-white/5">
          ...
          <div className="font-mono text-sm font-bold text-white">{stats.matchesCount}回</div>
        </div>
        <div className="space-y-0.5">
          <span className="text-[9px] text-zinc-500 uppercase tracking-widest font-semibold font-mono">Cleared</span>
          <div className="font-mono text-sm font-bold text-[#D4AF37]">{stats.tilesCleared}個</div>
        </div>
      </div>
    </div>
  );
